...
float gz_offset;  // Z 轴陀螺仪零漂，上电静止标定后用于 yaw 积分修正。
...
void MPU6050_CalibrateGyroZ(uint16_t times)
{
	uint16_t i;
	float sum = 0;
...
void MPU6050_YawReset(void)
{
	// 没有磁力计时，yaw 是相对角度；清零表示把当前车头方向当作 0 度。
	yaw_gyro = 0;
	yaw_Kalman = 0;
}
